//originally from https://github.com/mjansen4857/nt4/blob/main/lib/src/nt4_client.dart
//I made some small modifications to have greater type safety and such
...
class NT4Client {
  ...
  Future<T?> subscribeAndRetrieveData<T>(
    String topic, {
    Duration timeout = const Duration(seconds: 2, milliseconds: 500),
  }) async {
  ...
  void _wsConnect() async {
    ...
    _mainWebsocket = WebSocketChannel.connect(
  ...
  Future<void> _rttConnect() async {
    ...
    _rttWebsocket = WebSocketChannel.connect(
  ...
  void _wsOnMessage(data) {
    if (data is String) {
      ...
      for (var msg in rxArr) {
        ...
        if (method == 'announce') {
          if (params["name"] == null ||
              ...
              params["properties"] is! Map<String, dynamic>) {
            //ignore invalid messages
            continue;
          }

          late final int pubUID;
          if (params["pubid"] != null) {
            if (params["pubid"] is! int) {
              //ignore invalid messages
              continue;
            }
            pubUID = params["pubid"] as int;
          } else {
          ...
          if (params["name"] == null ||
              params["name"] is! String ||
              params["update"] == null ||
              params["update"] is! Map<String, dynamic>) {
            //ignore invalid messages
            continue;
          }

          var topicName = params['name'] as String;
          ...
          Map<String, dynamic> update =
              params['update'] as Map<String, dynamic>;
      ...
      var u = Unpacker.fromList(List.from(data));
